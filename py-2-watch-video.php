...
$sql = "SELECT pynum FROM users WHERE id = $id";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
$current_number = $row["pynum"];
if($current_number >= 2)
{
   echo "";
}
else
{
   ...
   $sql = "UPDATE users SET pynum = $new_number WHERE id = $id";
...
header("Location: py-3-watch-video.php");
}

$id = $_SESSION["id"];
$sql = "SELECT * FROM users WHERE id = $id AND pynum >= '2'";
$result = mysqli_query($conn, $sql);
...
$sql = "SELECT * FROM comment_py WHERE p2 = 1 ORDER BY comment_id DESC";
...
<section class="watch-video">
   ...
   <div class="video-container">
      <div class="video">
         <video src="images/Python/py-vid-2.mp4" controls poster="images/js.jpg" id="video"></video>
      </div>
      <h3 class="title">Python Programming Tutorial (part 02)</h3>
      <div class="info">
      ...
      <form action="" method="post" class="flex" enctype="multipart/form-data">
         ...
         <a class= "inline-btn" id="update-button" href="py-3-watch-video.php" name="update-button">Save and Next</a>
...
<section class="comments">
   ...
   <form action="php/comment-create-py.php" 
      ...
      <input type="hidden" value="0" class="inline-btn" name="p1">
      <input type="hidden" value="1" class="inline-btn" name="p2">
      <input type="hidden" value="0" class="inline-btn" name="p3">
