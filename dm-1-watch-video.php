...
$sql = "SELECT dmnum FROM users WHERE id = $id";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
$current_number = $row["dmnum"];
if($current_number >= 1)
{
   echo "";
}
else
{
   ...
   $sql = "UPDATE users SET dmnum = $new_number WHERE id = $id";
...
header("Location: dm-2-watch-video.php");
}

$id = $_SESSION["id"];
$sql = "SELECT * FROM users WHERE id = $id AND dmnum >= '1'";
$result = mysqli_query($conn, $sql);
...
$sql = "SELECT * FROM comment_dm WHERE dm1 = 1 ORDER BY comment_id DESC";
...
<section class="watch-video">
   ...
   <div class="video-container">
      <div class="video">
         <video src="images/DM/DM-vid-1.mp4" controls poster="images/js.jpg" id="video"></video>
      </div>
      <h3 class="title">Digital Marketing Tutorial (part 01)</h3>
      <div class="info">
      ...
      <form action="" method="post" class="flex" enctype="multipart/form-data">
         ...
         <a class= "inline-btn" id="update-button" href="dm-2-watch-video.php" name="update-button">Save and Next</a>
...
<section class="comments">
   ...
   <form action="php/comment-create-dm.php" 
      ...
      <input type="hidden" value="1" class="inline-btn" name="dm1">
      <input type="hidden" value="0" class="inline-btn" name="dm2">
      <input type="hidden" value="0" class="inline-btn" name="dm3">
