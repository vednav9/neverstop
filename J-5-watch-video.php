...
$sql = "SELECT jnum FROM users WHERE id = $id";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
$current_number = $row["jnum"];
if($current_number >= 5)
{
   echo "";
}
else
{
   ...
   $sql = "UPDATE users SET jnum = $new_number WHERE id = $id";
...
header("Location: J-6-watch-video.php");
}

$id = $_SESSION["id"];
$sql = "SELECT * FROM users WHERE id = $id AND jnum >= '5'";
$result = mysqli_query($conn, $sql);
...
$sql = "SELECT * FROM comment_j WHERE j5 = 1 ORDER BY comment_id DESC";
...
<section class="watch-video">
   ...
   <div class="video-container">
      <div class="video">
         <video src="images/Java/java-vid-5.mp4" controls poster="images/java.jpg" id="video"></video>
      </div>
      <h3 class="title">Java Programming (part 05)</h3>
      <div class="info">
      ...
      <form action="" method="post" class="flex" enctype="multipart/form-data">
         ...
         <a class= "inline-btn" id="update-button" href="J-6-watch-video.php" name="update-button">Save and Next</a>
...
<section class="comments">
   ...
   <form action="php/comment-create-j.php" 
      ...
      <input type="hidden" value="0" class="inline-btn" name="j1">
      <input type="hidden" value="0" class="inline-btn" name="j2">
      <input type="hidden" value="0" class="inline-btn" name="j3">
      <input type="hidden" value="0" class="inline-btn" name="j4">
      <input type="hidden" value="1" class="inline-btn" name="j5">
      <input type="hidden" value="0" class="inline-btn" name="j6">
